...
export async function getChatterPanelCreditsViaId(
  chatterId: string,
  streamerId: string
) {
...
export async function getChatterPanelCreditsViaLogin(
  chatterLogin: string,
  streamerId: string
) {
  return await prisma.chatterPanelCredits.findFirst({
...
export async function createChatterCreditsProfile(
  profile: Pick<ChatterPanelCredits, "chatterId" | "streamerId" | "credits">
) {
...
export async function updateChatterCreditsViaId(
  profile: Pick<ChatterPanelCredits, "chatterId" | "streamerId" | "credits">
) {
  const user = await getChatterPanelCreditsViaId(
    profile.chatterId,
    profile.streamerId
  );
