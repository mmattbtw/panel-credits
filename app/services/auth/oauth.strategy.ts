...
import { createChatter, getChatterViaId } from "../db/chatter.server";
import { createStreamer, getStreamerViaId } from "../db/streamer.server";
  ...
  > extends Strategy<User, OAuth2StrategyVerifyParams<Profile, ExtraParams>> {
    ...
    protected async userProfile(
      ...
      let [user, streamer] = await Promise.all([
        getChatterViaId(json.data[0].id),
        getStreamerViaId(json.data[0].id),
      ])

      if (!user) {
        user = await createChatter({
          id: json.data[0].id,
